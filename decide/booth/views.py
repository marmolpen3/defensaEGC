...
def last_12_months_votings_user(list_vid):
    ...
    participacion_meses = Vote.objects.filter(id__in=list_vid).filter(voted__range=(start,end)).annotate(month=ExtractMonth('voted')).values('month').annotate(participacion=Count('id'))

    for v in votaciones_meses:
        months[v['month']-1] = v['votaciones']
    for p in participacion_meses:
        participations[p['month']-1] = p['participacion']
